...
### 前提
- ログイン済みの状態（test@example.com / test123 など）

### ステップ1: プロフィール画面へ遷移
- **ツール**: `mcp__playwright__browser_click`
- **要素**: プロフィール設定ボタン
- **期待結果**: プロフィール編集画面が表示される

### ステップ2: パスワード変更フォーム入力
1. **現在のパスワード入力**
   - **ツール**: `mcp__playwright__browser_type`
   - **要素**: 現在のパスワード入力フィールド
   - **値**: 現在のパスワード（例: test123）
2. **新しいパスワード入力**
   - **ツール**: `mcp__playwright__browser_type`
   - **要素**: 新しいパスワード入力フィールド
   - **値**: 新しいパスワード（例: newpass123）
3. **新しいパスワード（確認）入力**
   - **ツール**: `mcp__playwright__browser_type`
   - **要素**: 新しいパスワード（確認）入力フィールド
   - **値**: 新しいパスワード（例: newpass123）
4. **変更ボタンクリック**
   - **ツール**: `mcp__playwright__browser_click`
   - **要素**: パスワード変更ボタン
...
### ステップ3: 成功メッセージ確認
- **期待結果**: 「パスワードが変更されました」等のメッセージが表示される

### ステップ4: ログアウト→新パスワードで再ログイン
1. **ログアウト**
   - **ツール**: `mcp__playwright__browser_click`
   - **要素**: ログアウトボタン
2. **新パスワードでログイン**
   - **ツール**: `mcp__playwright__browser_type`/`mcp__playwright__browser_click`
   - **値**: 新しいパスワードでログイン
3. **期待結果**: 正常にログインできる

### ステップ5: 旧パスワードでのログイン拒否
- **期待結果**: 旧パスワードではログインできず、エラーメッセージが表示される
